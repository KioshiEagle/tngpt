...
def run_rag_pipeline(config, question, retriever):
    ...
    results = retriever.search(question, top_k=top_k, similarity_threshold=similarity_threshold)
    ...
    try:
        ...
        err_msg = str(e).lower()
        if "does not exist" in err_msg or "not found" in err_msg or "invalid" in err_msg:
            logger.warning(f"Modèle {model_name} invalide sur Groq, PRUNING de l'essai.")
...
def judge_rag_quality(context, question, response, evaluate_persona=True):
    ...
    persona_rule = "- 'persona_compliance' : 1.0 si le ton est bien celui d'un étudiant décontracté, sinon 0.0." if evaluate_persona else ""
...
def evaluate_response(context, question, response, ground_truth="", num_judgments=1, evaluate_persona=True):
    # Juge Principal (Qualité)
    accumulated_scores = {k: 0.0 for k in ["faithfulness", "answer_relevance", "persona_compliance", "safety", "answer_correctness"]}
    valid_judgments = 0
    ...
    for _ in range(num_judgments):
        ...
        if scores:
            ...
            accumulated_scores["answer_relevance"] += float(scores.get("answer_relevance", 0.0))
            if evaluate_persona:
                accumulated_scores["persona_compliance"] += float(scores.get("persona_compliance", 0.0))
    ...
    if ground_truth:
        import rapidfuzz
        exact_match_score = rapidfuzz.fuzz.token_set_ratio(ground_truth.lower(), response.lower())
        if exact_match_score >= 90:
            accumulated_scores["answer_correctness"] = 1.0
        else:
            accumulated_scores["answer_correctness"] = judge_correctness(question, response, ground_truth)
...
def objective(trial, retriever):
    ...
    embedding_model = trial.suggest_categorical("embedding_model", ["miniLM", "e5-small"])
    top_k = trial.suggest_categorical("top_k", [2, 3, 5, 8])
    similarity_threshold = trial.suggest_float("similarity_threshold", 0.2, 0.8)
    generation_prompt = trial.suggest_categorical("generation_prompt", ["strict", "concise", "persona", "citation"])
    generation_temperature = trial.suggest_categorical("generation_temperature", [0.0, 0.2, 0.5, 0.7])
    generation_llm = trial.suggest_categorical("generation_llm", ["llama-3.1-8b-instant", "gemma2-9b-it", "mixtral-8x7b-32768", "qwen-2.5-32b"])
    ...
    metrics_sum = {
        ...
        "persona_compliance": 0.0, "safety": 0.0,
        "answer_correctness": 0.0, "retrieval_mrr": 0.0, "retrieval_ndcg": 0.0
    }
    ...
    for i, item in enumerate(GOLDEN_DATASET):
        ...
        while not success and retries > 0:
            try:
                ...
                response, context_extrait, retrieved_sources = run_rag_pipeline(config, question, retriever)
                ...
                if expected_docs:
                    ...
                    dcg = sum(1.0 / math.log2(i + 1) for i, src in enumerate(retrieved_sources, 1) if src in expected_docs)
                ...
                scores = evaluate_response(context_extrait, question, response, ground_truth, num_judgments=1, evaluate_persona=has_persona)
    ...
    for k, total in metrics_sum.items():
        ...
        if total == 0.0 and k in ["answer_correctness", "retrieval_mrr", "retrieval_ndcg", "persona_compliance"]:
            continue
    ...
    if len(study.trials) > 0:
        try:
            ...
            for i, trial in enumerate(best_trials, 1):
                ...
                logger.info(f"  Qualité : {trial.values[0]:.4f} | Latence : {trial.values[1]:.2f}s")
